...
internal sealed class NullableLessThanRule<T> : ValidationRuleBase<T?>
	where T : struct, IComparable<T> {
	private readonly T _compareValue;

	public NullableLessThanRule( T compareValue ) {
	...
	protected override string GetDefaultMessage( T? value ) {
		return value.HasValue
			? $"The value '{value.Value}' must be less than '{_compareValue}'."
			: $"The value must be less than '{_compareValue}'.";
	}
}
