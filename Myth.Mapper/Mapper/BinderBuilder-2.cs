...
namespace Myth.Morph {
	...
	/// </summary>
	public class BinderBuilder<TSource, TDestination> {
		...
		public BinderBuilder<TSource, TDestination> Bind<TMember>(
		...
		internal BinderBuilder<TSource, TDestination> BindInternal<TMember>(
			Expression<Func<TDestination, TMember>> destination,
			Func<TSource, IServiceProvider, TMember> resolver ) {
			if ( destination.Body is not MemberExpression memberExp || memberExp.Member is not MemberInfo member )
				throw new ArgumentException( "Expressão inválida para destino." );
		...
		public BinderBuilder<TSource, TDestination> BindAsync<TMember>(
			Expression<Func<TDestination, TMember>> destination,
		...
		internal async Task ApplyAsync( TSource src, TDestination dest, IServiceProvider sp ) {
			...
			AutoBind( src, dest, sp );
		}

		public BinderBuilder<TSource, TDestination> Ignore<TValue>( Expression<Func<TDestination, TValue>> destSelector ) {
			if ( destSelector.Body is MemberExpression member )
				_ignoredProperties.Add( member.Member.Name );
			return this;
		}

		private void AutoBind( TSource src, TDestination dest, IServiceProvider sp ) {
		...
		private object? MapValue( object srcValue, Type srcType, Type destType, IServiceProvider sp ) {
			...
			var registry = ( MorphRegistry? )sp.GetService( typeof( MorphRegistry ) );
			if ( registry != null ) {
				try {
					// Verifica se tem mapeamento registrado
					if ( registry.HasMapping( srcType, destType ) ) {
						var mapMethod = typeof( MorphRegistry )
							.GetMethod( "Morph" )?
					...
					Debug.WriteLine( $"[Morph] Falha no mapeamento via registry de {srcType.Name} para {destType.Name}: {ex.Message}" );
				}
			}
